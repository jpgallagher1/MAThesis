...
\begin{corollary}
    Let $J$ be a cantor set.  Then there exists a set $S \subset [0,1]$ of full measure that does not contain an affine copy of $E$. 
\end{corollary}
\begin{proof}
    Let $J$ be a Cantor set.  Then by theorem \ref{theorem_positive_NW}, there exists a dense $G_\delta$ that does not contain an affine copy of $J$.  

    Finally by taking a countable disjoint union of translations of $\bigcup_k G_k$ we can construct a set of arbitraty size such that $\sum \ell(G_k) = 1.$ 
\end{proof}
