...
Exploring this notion a little more deeply, we can begin to investigate which patters appear everywhere.  Informally, a set is called universal in another set, when every subset of the larger set contains some scaled and translated copy of original.  In the context of a measurable set:
...
\end{definition}
  Paul Erdos proposed a conjecture (\ref{ErdConj}) that no infinite set, is universal in the the collection of sets with positive measure.  We will explore an analogous problem in a topological setting. 

    %Patterns in Data sets
    %Def Affine Copy
    %Def Measure-Universal
    %State Erdos Conjecture (only need to focus on infinite sequences that decay to zero.  The rest follows)
